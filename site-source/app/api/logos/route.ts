...
export async function POST(request: Request) {
  try {
    ...
    if ((request.headers.get("content-type") || "").includes("application/json")) {
      ...
      await getLogoBucket().put(logoStorageKey(id), imported.bytes, {
        httpMetadata: {
          contentType: imported.contentType,
          cacheControl: "public, max-age=31536000, immutable",
        },
        customMetadata: { importedFrom: imported.source.slice(0, 1_000) },
      });
    ...
    await getLogoBucket().put(logoStorageKey(id), file.stream(), {
      httpMetadata: {

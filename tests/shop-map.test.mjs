...
test("supports adaptive branding, direct logo uploads, and typed contacts", async () => {
  const [page, styles, shops, uploadRoute, logoRoute, logoStorage] = await Promise.all([
    ...
    readFile(new URL("lib/logo-storage.ts", root), "utf8"),
  ]);
  ...
  assert.match(uploadRoute, /storeLogo/);
  ...
  assert.match(logoRoute, /downloadLogo/);
  assert.match(logoStorage, /getSupabaseAdmin\(\)\.storage\.from/);
  assert.match(logoStorage, /\.upload\(/);
  assert.match(logoStorage, /\.download\(/);
});
...
test("supports inline persistent prospect tracking without opening Edit", async () => {
  ...
  assert.match(schema, /visit_status\s+text\s+not null default 'not_visited'/i);
  assert.match(schema, /reach_out_sent\s+boolean\s+not null default false/i);
  assert.match(schema, /followup_sent\s+boolean\s+not null default false/i);
  assert.match(schema, /manual_notes\s+text\s+not null default ''/i);
  assert.match(server, /visitStatus: row\.visit_status === "visited" \? "visited" : "not_visited"/);
  assert.match(server, /manualNotes: row\.manual_notes \|\| undefined/);
...
test("ships a locked-down Supabase schema and Netlify configuration", async () => {
  const [schema, supabaseClient, logoStorage, netlify, envExample, packageJson] = await Promise.all([
    readFile(new URL("supabase/schema.sql", root), "utf8"),
  ...
  assert.match(schema, /create table if not exists public\.shops/i);
  assert.match(schema, /create table if not exists public\.app_state/i);
  assert.match(schema, /enable row level security/i);
  assert.match(schema, /revoke all on table public\.shops from anon, authenticated/i);
  assert.match(schema, /grant select, insert, update, delete on table public\.shops to service_role/i);
  assert.match(schema, /'shop-logos'/);
  assert.match(schema, /airport/);
  assert.match(schema, /train_station/);
  assert.match(supabaseClient, /SUPABASE_SECRET_KEY/);
  assert.match(supabaseClient, /SUPABASE_SERVICE_ROLE_KEY/);
  assert.match(logoStorage, /storage\.from\(SHOP_LOGO_BUCKET\)/);
  ...
  assert.ok(packageData.dependencies.next);
  assert.ok(packageData.dependencies["@supabase/supabase-js"]);
  assert.equal(packageData.dependencies.vinext, undefined);
  assert.equal(packageData.dependencies["drizzle-orm"], undefined);
});
